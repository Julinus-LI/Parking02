...
<body>
				...
				<form action="<%=path%>/PageListServlet02?currentPage=1&type=user"
					...
					<table class="search-tab">
						<tr>
							...
							<td><input class="common-text" placeholder="关键字"
								name="jdate" value="" id="" type="text" class="Wdate"
 ...
 --%>
						...
						<c:if test="${pagebean.totalPage == 1} ">
							<a href="PageListServlet02?currentPage=1&type=user">首页</a>
						</c:if>

					<%-- 	<c:if test="${pagebean.totalPage > 1} "> --%>
						<c:if test="${pagebean.totalPage > 1}"> 
							<c:if test="${pagebean.currentPage != 1} ">
								<a href="<%=path%>/PageListServlet02?currentPage=1&type=user">首页</a>
 								|   <a
									href="<%=path%>/PageListServlet02?currentPage=${pagebean.currentPage-1}&type=user">上一页</a>
							</c:if>
 							&nbsp;&nbsp;
 							...
 							<c:forEach begin="1" end="${pagebean.totalPage}" var="i">
								...
								<c:if test="${pagebean.currentPage != i}">
									<a
										href="<%=path%>/PageListServlet02?currentPage=${i}&type=user">${i }</a>
								</c:if>
							</c:forEach>	
							&nbsp;&nbsp;
 							...
 							<c:if test="${pagebean.currentPage == pagebean.totalPage }">
								<a
									href="<%=path%>/PageListServlet02?currentPage=${pagebean.currentPage-1}&type=user">上一页</a>
							     	|  <a
									href="<%=path%>/PageListServlet02?currentPage=1&type=user">首页</a>
							</c:if>

							<c:if test="${pagebean.currentPage != pagebean.totalPage }">
								<a
									href="<%=path%>/PageListServlet02?currentPage=${pagebean.currentPage+1}&type=user">下一页</a>
							     	|  <a
									href="<%=path%>/PageListServlet02?currentPage=${pagebean.totalPage}&type=user">尾页</a>
							</c:if>
						</c:if>
