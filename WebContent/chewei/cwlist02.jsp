...
<body>
	...
	<div class="main-wrap">
		...
		<div class="search-wrap">
			<div class="search-content">
				<form action="<%=path%>/PageListServlet02?currentPage=1&type=status" method="post">
					...
					<table class="result-tab" width="100%">
						...
						<c:forEach items="${pagebean.list}" var="chewei">
							<tr align="center">
								...
								<td>${chewei.info}</td>
								<td>
									<c:if test="${chewei.chepai == Empty}">空闲</c:if>
									<c:if test="${chewei.chepai != Empty}">${chewei.chepai}</c:if>
								</td>
 ...
 --%>
						...
						<c:if test="${pagebean.totalPage == 1} ">
							<a href="PageListServlet02?currentPage=1&type=status">首页</a>
						</c:if>

					<%-- 	<c:if test="${pagebean.totalPage > 1} "> --%>
						<c:if test="${pagebean.totalPage > 1}"> 
							<c:if test="${pagebean.currentPage != 1} ">
								<a href="<%=path%>/PageListServlet02?currentPage=1&type=status">首页</a>
 								|   <a
									href="<%=path%>/PageListServlet02?currentPage=${pagebean.currentPage-1}&type=status">上一页</a>
							</c:if>
 							&nbsp;&nbsp;
 							...
 							<c:forEach begin="1" end="${pagebean.totalPage}" var="i">
								...
								<c:if test="${pagebean.currentPage != i}">
									<a
										href="<%=path%>/PageListServlet02?currentPage=${i}&type=status">${i }</a>
								</c:if>
							</c:forEach>	
							&nbsp;&nbsp;
 							...
 							<c:if test="${pagebean.currentPage == pagebean.totalPage }">
								<a
									href="<%=path%>/PageListServlet02?currentPage=${pagebean.currentPage-1}&type=status">上一页</a>
							     	|  <a
									href="<%=path%>/PageListServlet02?currentPage=1&type=status">首页</a>
							</c:if>

							<c:if test="${pagebean.currentPage != pagebean.totalPage }">
								<a
									href="<%=path%>/PageListServlet02?currentPage=${pagebean.currentPage+1}&type=status">下一页</a>
							     	|  <a
									href="<%=path%>/PageListServlet02?currentPage=${pagebean.totalPage}&type=status">尾页</a>
							</c:if>
						</c:if>
