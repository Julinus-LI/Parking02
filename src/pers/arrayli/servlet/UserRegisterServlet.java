...
public class UserRegisterServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(result){
				out.
		println("<script>alert('注册成功请登录！');window.location.href='login.jsp'</script>");
				
			}else{ 		// 如果注册失败
				out.
println("<script>alert('注册成功失败,请重新注册！');window.location.href='userinfo/regUserinfo.jsp'</script>");
			}
